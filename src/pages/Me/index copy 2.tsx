...
import { getTest, testVersion } from '@/services/test'
import { useRequest as ajax } from 'ahooks';
console.log(' useModel ： ', useModel   )// 

const ajaxModel = () => ajax(testVersion, {
...
function Me(defProps) {
  ...
  // console.log(' testModel ： ', testModel, testModel.increment({name: 'zyb',})   )
  // console.log(' testModel ： ', testModel, testModel.increment([{name: 'zyb',}, 888])   )
  // console.log(' testModel ： ', testModel, testModel.increment({name: 'zyb666',}), testModel.decrement({name: 'zyb',}), testModel.decrement()   )
  const meModel = useModel('me');
  // const testModelObj = testModel.optionRepoAsync()
  useEffect(() => {
    console.log(' testModel 加载 ： ', testModel,    )
    // console.log(' testModelObj ： ', testModel, testModelObj   )
    // ajaxModel.run(
    //   // { attr: 'xxx', }
    // )
    // testModelObj.run({
    //   attr: 'xxx',
    // })
    // console.log(' meModel ： ', meModel, meModel.getRepoAsync()   )
    // getTest()
    // ajax()
  }, [])
  ...
  const url = '/datapageaccesslog/dataPageAccessLog/rankingUser'
  ...
  const modalHtml = `<p>访问次数：用户进入经营驾驶舱即算一次，5分钟之内多次进入不重复计数</p>
  ...
  const menusData = [
    {
      title: '皮肤中心',
      imgUrl: require('@/assets/me/skin.svg'),
      router: '/skinCenter',
    },
    {
      title: '版本信息',
      imgUrl: require('@/assets/me/version.svg'),
      router: '/version',
    },
    {
      title: '权限说明',
      imgUrl: require('@/assets/me/power.svg'),
      router: '/power',
    },
    {
      title: '问题帮助',
      imgUrl: require('@/assets/me/help.svg'),
      router: '/help',
    },
    {
      title: '用户体验调研',
      imgUrl: require('@/assets/me/question.svg'),
  ...
  function handleOnClickSort() {
    ...
    if (currentData) {
      ...
      newData.rankingVoList.sort(function (a, b) {
        if (order === 1) {
    ...
    <>
        ...
        <div className={style.summary}>
          <div>
            <div className={style.number}>
              {currentData && currentData.visitNumber}
            </div>
            <div>月访问次数</div>
          </div>
          <div>
            <div className={style.number}>
              {currentData && currentData.visitTime}
            </div>
            <div>月访问时长</div>
          </div>
          <div>
      ...
      <div className={style.ranking}>
        ...
        <div className={style.rankingListBox}>
          {currentData &&
            currentData.rankingVoList.map((item, index) => {
              return (
                <div
                  className={
                    currentData.userId === item.userId
          ...
          title={
            ...
            >
              ...
              {getNewsNum(list) === 0 ? (
                ...
                <Badge content={getNewsNum(list)} bordered />
              )}
            </div>
          }
          extra={<RightOutline />}
