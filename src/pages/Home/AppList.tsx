...
export function AppList() {
  ...
  const { followList, updateFollowList, token } = user
  ...
  useEffect(() => {
    if (!inited.current) {
      inited.current = true
      setIndicatorIds(
        Array.from({ length: followList.length }, (v, k) => {
          return { id: followList[k].id, index: k + 1 }
        })
      )
    }
  }, [followList])
  ...
  const onRemove = useCallback(
    async ({ index }) => {
      ...
      sendBuriedPoint(
        ...
        '取消关注',
        moment(new Date()).format('YYYY-MM-DD hh:mm:ss'),
        `${followList[index - 1].title} 取消关注`
      )
      const removed = indicatorIds.find((item) => item.index === index)
      const nextIndicators = indicatorIds.filter((item) => item.index !== index)
      setIndicatorIds(nextIndicators)
      unfollow({ id: removed.id } as any)
    },
    [unfollow, indicatorIds]
  )
          ...
          <Card
            ...
            extra={
              <>
                {editing && (
                  <div
                    style={{ color: 'rgba(110, 148, 242, 1)' }}
                    onClick={() => {
                      setEditing(false)
                    }}
                  >
              ...
              renderItem={({
                ...
                return (
                  ...
                  >
                    ...
                    >
                      {(() => {
                        const indicatorId = followList[index]
                        const sort = indicatorIds.find(
                          (item) => item.index === index + 1
                        )
                        const item = followList.find(
