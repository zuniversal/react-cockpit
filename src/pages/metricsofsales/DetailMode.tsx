import { WaterMark } from 'antd-mobile';
import React, { useState, useEffect, useMemo, useRef } from 'react';

import { useMatch, useSearchParams } from 'react-router-dom';

import { Loading } from '../../components/loading/Loading';
import { useCurrentApp } from '../../contexts/apps/CurrentAppContext';

import { useQuery } from '../../hooks/useQuery';
import style from './Detail.module.less';
import { DetailChart } from './DetailChart';
import { DetailTable } from './DetailTable';

export function DetailMode() {
  // 柱状图，表格共用的参数
  const [tabKey, setTabKey] = useState('1');
  const [search, setSearch] = useSearchParams();
  const applicationArea = search.get('applicationArea') ?? '';
  const [chooseName, setChooseName] = useState('');
  const [chooseNameType, setChooseNameType] = useState('');
  const [title, setTableTitle] = useState('全部');
  const [visibleSaleType, setVisibleSaleType] = useState<
    'salesVolume' | 'salesQuantity'
  >('salesVolume');
  const view = search.get('view') === 'table' ? 'table' : 'detail';

  const isTableLoading = false;

  const { user } = useCurrentApp();
  const { userInfo } = user;
  const { realname, username } = userInfo.userInfo;
  const textProps = {
    content: `${realname} ${username.substring(
      username.length - 4,
      username.length,
    )}`,
    gapX: 100,
    gapY: 100,
    fontSize: 12,
    fontColor: 'rgba(220, 221, 225, .6)',
    rotate: 0,
  };

  const [props, setProps] = useState<{ [key: string]: any }>(textProps);

  const {
    error: error1,
    data: selectSaleDeliveryTypeData,
    query: query1,
  } = useQuery('/saleForecast/selectSaleDeliveryType');

  useEffect(() => {
    /**
     * 日期参数(chooseDate)和年月日(dateType)参数都已经封装好了，不用传
     */
    query1({ chooseName, chooseNameType, applicationArea });
  }, [query1, chooseName, chooseNameType, applicationArea]);

  const [salesQuantitySum, salesVolumeSum, sortableColumns] = useMemo(() => {
    let a = 0;
    let b = 0;
    const temp = [];
    if (selectSaleDeliveryTypeData) {
      selectSaleDeliveryTypeData.map((item, index) => {
        a += item.salesQuantity;
        b += item.salesVolume;
        temp.push({ deliveryType: item.deliveryType });
      });
    }
    return [a, b, temp];
  }, [selectSaleDeliveryTypeData]);

  const data1 = useMemo(() => {
    const temp = [];
    if (selectSaleDeliveryTypeData) {
      try {
        if (visibleSaleType === 'salesQuantity') {
          selectSaleDeliveryTypeData.map((item, index) => {
            temp.push({
              type: item.deliveryType,
              value: item.salesQuantity,
              percent:
                salesQuantitySum === 0
                  ? 0
                  : (item.salesQuantity / salesQuantitySum) * 100,
            });
          });
        } else {
          selectSaleDeliveryTypeData.map((item, index) => {
            temp.push({
              type: item.deliveryType,
              value: item.salesVolume,
              percent:
                salesVolumeSum === 0
                  ? 0
                  : (item.salesVolume / salesVolumeSum) * 100,
            });
          });
        }
      } catch (error) {}
    }
    const temp1 = temp.sort(function (a, b) {
      return b.value - a.value; //实按降序排序
    });

    return temp1;
  }, [selectSaleDeliveryTypeData, visibleSaleType]);

  if (view === 'table') {
    if (isTableLoading) {
      return <Loading />;
    }

    return (
      <div className={style.detailBox}>
        <DetailTable
          title={title}
          chooseNameType={chooseNameType}
          chooseName={chooseName}
          sortableColumns={sortableColumns}
        />
        <WaterMark {...props} />
      </div>
    );
  } else {
    return (
      <div className={style.detailBox}>
        <DetailChart
          visibleSaleType={visibleSaleType}
          setVisibleSaleType={setVisibleSaleType}
          tabKey={tabKey}
          setTabKey={setTabKey}
          setChooseNameType={setChooseNameType}
          chooseName={chooseName}
          setChooseName={setChooseName}
          setTableTitle={setTableTitle}
          data1={data1}
        />
        <WaterMark {...props} />
      </div>
    );
  }
}

export default DetailMode;
