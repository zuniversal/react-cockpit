...
import { useCurrentApp, useUser } from '@/tamp';
import PcTips from '@/components/PcTips';

function Login() {
  const { getUserInfoAsync, user } = useModel('user');
  const { setToken, getFailTimes } = user;
    ...
    async (args) => {
      ...
      try {
        ...
        if (json.success) {
          ...
          console.log(' getUserInfoAsync ： ', getUserInfoAsync);
