...
export function EchartsMode2(props) {
  ...
  data &&
    data.map((item) => {
      title.push(item.vehicleModel)
      marketShareData.push(accMul(item.marketShare || 0, 100))
      permeabilityData.push(accMul(item.permeability || 0, 100))
      tooltip[item.vehicleModel] = item
    })
    ...
    xAxis: [
      {
        ...
        axisLabel: {
          ...
          formatter(val: string) {
            if (val.length > 6) {
              const idx = val.length / 6
              const idxInt = Math.trunc(val.length / 6)
              let str = ''
              for (let i = 0; i < idxInt; i++) {
                if (i === 0) {
                  str += val.slice(0, 6) + '\n'
                } else {
                  str += val.slice(i * 6, i * 6 + 6) + '\n'
                }
              }
              if (idx !== idxInt) {
                str += val.slice(idxInt * 6)
              }
              return str
            }
            return val
  ...
  useEffect(() => {
    const node = document.getElementById('main1')
    setMain(node)
  }, [main])
