...
export function DetailTable(props) {
  ...
  }>('/marginalAmount/selectMarginalAmountDetailsForm')
  const [searchParams] = useSearchParams()
  const applicationArea = searchParams.get('applicationArea') ?? ''
  const [sort, setSort] = useState<'asc' | 'desc'>('desc')
  const orderByData =
    segmentKey === '1' ? 'marginalAmountStandard' : 'marginalAmountActual'
        ...
        onChange={(state) => {
          ...
          if (state.orderBy === orderBy) {
            ...
            sendBuriedPoint(
              ...
              moment(new Date()).format('YYYY-MM-DD hh:mm:ss'),
              `边际额 ${
                orderBy === 'marginalAmountActual' ? '默认' : '边际额'
              } ${sort === 'desc' ? '升序' : '降序'}`
            )
          } else {
            // 事件埋点
            sendBuriedPoint(
              ...
              moment(new Date()).format('YYYY-MM-DD hh:mm:ss'),
              `边际额 ${
                orderBy === 'marginalAmountActual' ? '边际额' : '默认'
              } ${sort === 'desc' ? '降序' : '升序'}`
            )
          }
          setSort(state.sort)
