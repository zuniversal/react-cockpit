...
import { MarginalColumn } from './DetailColumn'
                  ...
                  return (
                    <div
                      ...
                      onClick={() => {
                        ...
                        sendBuriedPoint(
                          '关注',
                          'metrics/metricsofmarginalamount/detail',
                          '维度切换',
...
export function DetailChart(props) {
  ...
  useEffect(() => {
    ...
    async function fn() {
      const { result } = await sendPagePoint({
        pageName:
          applicationArea === '' ? '边际额-总额' : `边际额-${applicationArea}`,
        ...
        requestUrl: '/marginalAmount/selectMarginalAmountDetails',
      })
      id = result
    }
    fn()
  ...
  const {
    error,
    data: selectMarginalAmountDetailsData,
    query,
  } = useQuery('/marginalAmount/selectMarginalAmountDetails')
  ...
  useEffect(() => {
     ...
     */
    const orderBy =
      key1 == '1' ? 'marginalAmountStandard' : 'marginalAmountActual'
  ...
  const columnData = useMemo(() => {
    const temp = []
    if (selectMarginalAmountDetailsData) {
      // 客户维度
      if (tabKey == '1') {
        try {
          setPages(
            selectMarginalAmountDetailsData.customerMarginalAmountDetailPage
              .pageCustomerList.pages
          )
          // 报价成本
          if (key1 == '1') {
            selectMarginalAmountDetailsData.customerMarginalAmountDetailPage.pageCustomerList.records.map(
              (item, index) => {
                temp.push({
      ...
      // 基地维度
        ...
        try {
          setPages(
            selectMarginalAmountDetailsData.entityMarginalAmountDetailPage
              .pageEntityList.pages
          )
          if (key1 == '1') {
            selectMarginalAmountDetailsData.entityMarginalAmountDetailPage.pageEntityList.records.map(
              (item, index) => {
                temp.push({
                  company: item.entity,
                  value: item.marginalAmountStandard,
    ...
    return temp
  }, [selectMarginalAmountDetailsData, tabKey])

  const isGraphLoading = !selectMarginalAmountDetailsData
  ...
  return (
    <>
      <HeadTitle>
        {applicationArea === '' ? '边际额' : `边际额-${applicationArea}`}
      </HeadTitle>
