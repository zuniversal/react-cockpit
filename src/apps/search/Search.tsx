...
export default function Search() {
  ...
  const [searchResult, setSearchResultFn] = useState({ groups: [], apps: [], indicatorList: [] })
  ...
  const [tabActive, setTabActive] = useState(localStorage.getItem(SEARCH_TAB_KEY))
    ...
    <div className={styles.searchWrapper} style={heightStyle}>
      ...
      >
        {searchResult.indicatorList.map((group, i) => {
                ...
                {tagItem.metricList.map(item => {
                  ...
                  const onFocus = () => {
                    ...
                    sendBuriedPoint(
                      ...
                      '取消关注',
                      moment(new Date()).format('YYYY-MM-DD hh:mm:ss'),
                      `${item.indicatorName} ${followed ? '取消关注' : '关注'}`
                    )
                  }
                  const btnStyle = {
                  ...
                  return <div className={styles.merticRow} key={item.id}>
                    ...
                    >
                      <span className={styles.title} onClick={() => goPage(item)}>
                        {item.indicatorName}
                      </span>
          ...
          {/* const title = <a className={'link'} href={`#${group.groupName}`}>{group.title}</a> */}
